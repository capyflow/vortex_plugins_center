...
func (g *Gateway) registerRoutes(root *http.VortexHttpRouterGroup) {
	...
	// 执行插件（简化后：不再需要 method 参数）
	pluginGroup.AddRouter([]string{http.MethodPost}, "/:id/execute", g.handleExecute)
...
// handleExecute 处理执行请求（简化后：不再需要 method 参数）
func (g *Gateway) handleExecute(ctx *http.Context) error {
	...
	resp, err := g.center.Execute(ctx.Context(), pluginID, params)
	if err != nil {
		return ctx.JsonResponse(http.Codes.InternalError, pkg.VMsgResponse{
			"error": err.Error(),
		})
